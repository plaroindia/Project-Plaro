// ================================================================
// follow_provider.dart  (UPDATED)
//
// Change from original:
//   - toggleFollow inserts a 'follow' notification to the target
//     user after a successful RPC call (unfollow does NOT notify).
//   - Uses NotificationsNotifier.insert() — static, no Ref needed.
// ================================================================
...
class FollowState {
  ...
  FollowState copyWith({
    ...
    bool clearLastFollowersUpdate = false,
    bool clearLastFollowingUpdate = false,
  }) =>
      FollowState(
        ...
        processingFollowRequests:
        processingFollowRequests ?? this.processingFollowRequests,
        lastFollowersUpdate: clearLastFollowersUpdate
            ? null
            : (lastFollowersUpdate ?? this.lastFollowersUpdate),
        lastFollowingUpdate: clearLastFollowingUpdate
            ? null
            : (lastFollowingUpdate ?? this.lastFollowingUpdate),
...
class FollowNotifier extends StateNotifier<FollowState> {
  ...
  // ── UPDATED: toggleFollow fires follow notification ──────
  ...
  Future<void> toggleFollow(String targetUserId) async {
    ...
    final countDelta = isCurrentlyFollowing ? -1 : 1;

    // Optimistic update
    final updatedFollowerCounts =
    Map<String, int>.from(state.followerCounts);
    updatedFollowerCounts[targetUserId] =
        (updatedFollowerCounts[targetUserId] ?? 0) + countDelta;

    state = state.copyWith(
      processingFollowRequests: {
        ...state.processingFollowRequests,
        targetUserId
      },
      followingStatus: {
      ...
      followerCounts: updatedFollowerCounts,
    );
    _updateCountsInLists(targetUserId, countDelta);
    ...
    try {
      ...
      state = state.copyWith(
        processingFollowRequests: Set.from(state.processingFollowRequests)
          ..remove(targetUserId),
        clearLastFollowersUpdate: true,
        clearLastFollowingUpdate: true,
        error: null,
      );
      ...
      final revertedFollowerCounts =
      Map<String, int>.from(state.followerCounts);
      revertedFollowerCounts[targetUserId] =
          (revertedFollowerCounts[targetUserId] ?? 0) - countDelta;
      ...
      state = state.copyWith(
        ...
        processingFollowRequests: Set.from(state.processingFollowRequests)
          ..remove(targetUserId),
        followerCounts: revertedFollowerCounts,
  ...
  Future<bool> toggleFollowWithDebounce(String targetUserId) async {
    final now = DateTime.now();
    final last = _lastToggleTime[targetUserId];
    if (last != null && now.difference(last) < _debounceDelay) return false;
  ...
  void _updateCountsInLists(String targetUserId, int increment) {
    state = state.copyWith(
      followers: state.followers
          .map((f) => f.user_id == targetUserId
          ? f.copyWith(
          followersCount: (f.followersCount ?? 0) + increment)
          : f)
          .toList(),
  ...
  Future<Map<String, bool>> batchCheckFollowing(List<String> userIds) async {
    ...
    try {
      ...
      final set = (response as List)
          .map((e) => e['followee_id'] as String)
          .toSet();
      return Map.fromEntries(
          userIds.map((id) => MapEntry(id, set.contains(id))));
    } catch (_) {
      return {};
    }
  }

  void seedFollowingStatus(Map<String, bool> statusMap) {
    if (statusMap.isEmpty) return;
    state = state.copyWith(
        followingStatus: {...state.followingStatus, ...statusMap});
  }
  ...
  Future<void> refreshFollowingStatus(String userId) async {
    if (state.followingStatus.containsKey(userId)) return;
    final map = await batchCheckFollowing([userId]);
    if (!mounted) return;
    final alreadySet = state.followingStatus.containsKey(userId);
    if (alreadySet) return;
    state = state.copyWith(
        followingStatus: {...state.followingStatus, ...map});
  }
  ...
  bool isFollowing(String userId) =>
      state.followingStatus[userId] ?? false;
  bool isProcessingFollow(String userId) =>
      state.processingFollowRequests.contains(userId);
...
StateNotifierProvider<FollowNotifier, FollowState>(
        (ref) => FollowNotifier());

final isFollowingProvider = Provider.family<bool, String>(
        (ref, userId) =>
    ref.watch(followProvider).followingStatus[userId] ?? false);
