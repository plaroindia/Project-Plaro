...
class _ByteCreateScreenState extends ConsumerState<ByteCreateScreen> {
          ...
          Padding(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                hint: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 8),
                  child: Text('Select content domain', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                ),
          ...
          Padding(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                hint: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 8),
                  child: Text('Select subdomain (optional)', style: TextStyle(color: Colors.grey[600])),
                ),
                dropdownColor: Colors.grey[850],
                icon: const Icon(Icons.arrow_drop_down, color: Colors.white),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              IconButton(onPressed: _addTag, icon: const Icon(Icons.add, color: Colors.blue)),
            ],
          ),
          if (_tags.isNotEmpty)
            Padding(
  ...
  Widget build(BuildContext context) {
    ...
    // Initialise video player when video is selected
    if (byteCreateState.selectedVideo != null && _videoController == null) {
      _initializeVideoPlayer(byteCreateState.selectedVideo!.path);
    }

    // ── FIX: ref.listen instead of addPostFrameCallback ───────────────────────
    // Prevents duplicate/stacked snackbars when state changes rapidly.
    ref.listen<String?>(
      byteCreateProvider.select((s) => s.error),
      (previous, next) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              action: SnackBarAction(
                ...
                onPressed: () {
                  ref.read(byteCreateProvider.notifier).clearError();
                  ScaffoldMessenger.of(context).hideCurrentSnackBar();
                },
              ),
            ),
    ...
    // ── Button state: busy while loading OR moderating ────────────────────────
    final bool isBusy  = byteCreateState.isLoading || byteCreateState.isModerating;
    final bool canPost = byteCreateState.selectedVideo != null && !isBusy;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // ── CHANGED: shows moderation status while checking ───────────────
          Padding(
              ...
              child: isBusy
                  ? Row(
                      ...
                      children: [
                        ...
                        Text(
                          byteCreateState.isModerating
                              ? (byteCreateState.moderationStatus ?? 'Checking…')
                              : 'Posting…',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Video Selection/Preview — unchanged
            Container(
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text('${(byteCreateState.uploadProgress * 100).toInt()}%',
