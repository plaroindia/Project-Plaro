...
class FeedItem {
  ...
  // authorUserId is the UUID of the content creator — used for the FollowButton.
  // The RPC may return this as 'userId', 'user_id', or 'authorId'; we try all variants.
  ...
  factory FeedItem.fromJson(Map<String, dynamic> j) => FeedItem(
    ...
    // Try multiple key names the RPC might use for the author UUID
    authorUserId: (j['userId'] ?? j['user_id'] ?? j['authorId'] ?? j['authorUserId'] ?? '') as String,
...
class PostFeedState {
  ...
  // Pending follow-status seed: consumed by home_page after each page load
...
class PostFeedNotifier extends StateNotifier<PostFeedState> {
  ...
  Future<void> loadPosts() async {
    ...
    try {
      ...
      await _seedFollowStatus(state.posts);
  ...
  Future<void> loadMorePosts() async {
    ...
    try {
      ...
      await _seedFollowStatus(newPosts);
  ...
  Future<void> _loadPostsDirectly() async {
    try {
      ...
      await _seedFollowStatus(finalPosts);
  ...
  /// Batch-checks follow status for every unique author in [posts] and stores
  /// the result in [followStatusCache]. home_page consumes this cache and
  /// pushes it into followProvider so every PostCard FollowButton renders
  /// correctly on first paint without an extra DB round-trip.
  Future<void> _seedFollowStatus(List<Post_feed> posts) async {
  ...
  /// Called by home_page once it has consumed the cache.
