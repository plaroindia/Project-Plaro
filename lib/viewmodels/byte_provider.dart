...
class BytesFeedState {
  ...
  // Pending follow-status seed: consumed by byte_viewer after each page load
  ...
  BytesFeedState copyWith({
    ...
    Map<String, bool>? followStatusCache,
  }) {
    return BytesFeedState(
      ...
      followStatusCache: followStatusCache ?? this.followStatusCache,
    );
  }
}
...
class ByteCreateNotifier extends StateNotifier<ByteCreateState> {
  ...
  Future<bool> createByte() async {
    ...
    try {
      ...
      // ── Step 1: Text moderation — caption + tags ──────────────────────────
      if (state.caption.isNotEmpty || state.tags.isNotEmpty) {
        ...
        try {
          ...
          debugPrint(
            '[Moderation] Byte text: nsfw=${textResult.nsfwScore.toStringAsFixed(3)} '
            'verdict=${textResult.verdict.name} wordlist=${textResult.fromWordlist}',
          );
          if (textResult.isBlocked) {
            state = state.copyWith(
              isLoading: false, isModerating: false, moderationStatus: null,
              error: textResult.reason ?? 'Your caption contains inappropriate content.',
            );
            return false;
          }
        } catch (e) {
          debugPrint('[Moderation] Byte text check error (non-fatal): $e');
        }
      ...
      // FIX: 'byte' has no matching source in the plaro_transactions CHECK
      // constraint so PlaroPointsService.awardPointsForContent now skips it
      // gracefully instead of throwing a constraint violation. The call is kept
      // here unchanged — the fix lives in streakandpoints_provider.dart.
      try {
      ...
      // FIX: capture domain BEFORE state = ByteCreateState() resets it to null.
      // Previously logByteCreated always received domain: null because the
      // full state reset happened on the very next line.
...
class BytesFeedNotifier extends StateNotifier<BytesFeedState> {
  ...
  Future<void> loadMoreBytes() async {
    ...
    try {
      ...
      await _seedFollowStatus(deduped);
    } catch (e) {
      state = state.copyWith(
        isLoadingMore: false,
  ...
  Future<void> _loadBytesDirect() async {
    try {
      ...
      final List<int> byteIds = (response as List).map((r) => r['byte_id'] as int).toList();
      Set<String> likedIds = {};
  ...
  // ── Follow-status batch seeding ──────────────────────────────────────────

  /// Batch-checks follow status for every unique author in [bytes] and stores
  /// the result in [followStatusCache]. byte_viewer consumes this cache and
  /// pushes it into followProvider so every FollowButton renders correctly
  /// on first paint without an extra DB round-trip.
  Future<void> _seedFollowStatus(List<Byte> bytes) async {
    final currentUser = _supabase.auth.currentUser;
    if (currentUser == null) return;
    final authorIds = bytes
        .map((b) => b.userId)
    ...
    try {
      final rows = await _supabase
          ...
          .inFilter('followee_id', authorIds);
      final following = (rows as List).map((r) => r['followee_id'] as String).toSet();
      final map = Map<String, bool>.fromEntries(
        authorIds.map((id) => MapEntry(id, following.contains(id))),
      );
      state = state.copyWith(followStatusCache: map);
    } catch (_) {}
  }

  /// Called by byte_viewer once it has consumed the cache.
  void clearFollowStatusCache() {
    if (state.followStatusCache.isEmpty) return;
    state = state.copyWith(followStatusCache: {});
  }
  ...
  Byte _feedItemToByte(Map<String, dynamic> item, bool isLiked) {
    ...
    return Byte.fromJson({
      ...
      'user_id':       (item['userId'] ?? item['user_id'] ?? item['authorId'] ?? item['authorUserId'] ?? '') as String,
