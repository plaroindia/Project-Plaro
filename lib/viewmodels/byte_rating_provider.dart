// ================================================================
// byte_rating_provider.dart  (UPDATED)
//
// Change from original:
//   - submitRating inserts a 'rating' notification to the byte
//     author after a successful new submission.
// ================================================================
...
class ByteRatingNotifier extends StateNotifier<ByteRatingState> {
  ...
  void _setAggregates(List<EducatorReview> reviews) {
    ...
    state = state.copyWith(
      ...
      isLoading: false,
      averageRating:
      reviews.map((r) => r.rating).reduce((a, b) => a + b) / n,
      totalRatings: n,
      avgAccuracy:
      reviews.map((r) => r.accuracyScore).reduce((a, b) => a + b) / n,
      avgAuthenticity:
      reviews.map((r) => r.authenticityScore).reduce((a, b) => a + b) / n,
      avgUsefulness:
      reviews.map((r) => r.usefulnessScore).reduce((a, b) => a + b) / n,
      distribution: dist,
    );
  }
  ...
  // UPDATED: notify byte author on new rating
  Future<bool> submitRating({
    ...
    try {
      final payload = {
        'educator_id': userId,
        'content_type': 'byte',
        'content_id_int': int.parse(byteId),
        'rating': rating,
      ...
      if (existingReviewId != null) {
        ...
        // Only notify on new rating
        _sendRatingNotification(raterId: userId, rating: rating);
      }
      await _awardPoints(
          reviewId: reviewId,
          accuracyScore: accuracyScore,
          usefulnessScore: authenticityScore);
      await loadReviews();
  ...
  Future<void> _sendRatingNotification({
    ...
    try {
      ...
      final byteRow = results[0] as Map<String, dynamic>?;
      final raterRow = results[1] as Map<String, dynamic>?;

      if (byteRow == null) return;
      final authorId = byteRow['user_id'] as String?;
      if (authorId == null || authorId == raterId) return;
      ...
      await NotificationsNotifier.insert(
        ...
        type: 'rating',
        title: 'Your Byte was rated $stars',
        body: '$raterName rated your Byte $rating out of 5',
        relatedType: 'byte',
        relatedId: byteId,
      );
    } catch (_) {}
